        ...
        extends DefaultHandler
{
    ...
    {
        if ("a".equals (qName) && "s".equals (currentAttribute) &&
            currentDay >= 0 && currentDay <= 4)
        {
            String[] values = currentSubject.split ("-");
            if (values.length > 1)
            {
                String[] grp = values[1].split ("/");
                currentGroup = Integer.parseInt (grp[0]);

                currentSubject = values[0];
            }
            else
            {
                Log.i ("liquid", "lol");
            ...
            Log.i ("liquid", "" + currentDay + " "+ currentLesson);
            Log.i ("liquid", shortName);

            DbWriter.insertLesson (currentDay,
    ...
    public void characters (char[] ch, int start, int length) throws SAXException
    {
        ...
        if ("span".equals (currentName) && "tytulnapis".equals (currentAttribute))
        {
            ...
            if(tableType == Type.CLASS)
            {
                DbWriter.insertClass (shortName, "");
                runnable.setTableName (shortName);
            }
